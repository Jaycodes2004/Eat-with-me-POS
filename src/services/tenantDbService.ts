...
// export export async function withMasterClient<T>(
...
// 		await client.query(
// 			`CREATE USER "${dbUser}" WITH PASSWORD '${dbPassword.replace(
// 				/'/g,
// 				"''"
// 			)}'`
// 		);
// 		// 2) Create tenant database from template tenant_test_001, owned by master user first
// 		await client.query(
// 			`CREATE DATABASE "${dbName}" OWNER "${MASTER_DB_USER}" TEMPLATE tenant_test_001`
// 		);
// 		// 3) Then transfer ownership of the tenant DB to the tenant user
...
export async function createTenantUserAndDatabase(opts: {
  ...
  return withMasterClient(async (client) => {
    ...
    // 2) Create tenant database from template tenant_db, owned by master user
    await client.query(
      `CREATE DATABASE "${dbName}" OWNER "${MASTER_DB_USER}" TEMPLATE tenant_db`
    );
